...
print(paste0("Min RMSE: ",rf.min.rmse,", at K = 6 or 8 and 50 trees"))
```
  + Predictions 
```{r}
rf.predictions <- predict(rand_tree_model, validate_predictors)
...
#will have to rerun the svm models because I did not save the rmse of models with differnet eposolins
```
```{r}
epsilons %>%
...
```
  ...
  + Predictions 
```{r}
smv.predictions <- predict(svmRTuned, validate_predictors)
svm.test.rmse <- RMSE(smv.predictions, validate$outcome)
svm.test.rmse
```